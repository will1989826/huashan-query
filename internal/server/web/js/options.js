...
export const RELEASES = [
  { v: '0.10.5', date: '2026-09-21', items: [
    'Windows 版会同时扫描旧版微信的默认网页登录缓存，以及新版微信网页进程声明的运行时和文件存储位置',
  ] },
  { v: '0.10.4', date: '2026-09-20', items: [
    '个人概览与完整逐场战绩的场次不一致时，会明确提示官方概览数据有误，并建议以逐场战绩为准',
  ] },
  { v: '0.10.3', date: '2026-09-17', items: [
    '个人数据新增“重新拉取数据”按钮，确认官方有更新时可就地重新联网获取该选手的最新数据',
    '多人对比可对每位选手单独重新拉取数据，只更新这一个人，不影响其他人',
    '赛事数据可在结果区重新拉取当前赛区、赛季和比赛类型的最新门派排名、均分、选手排名和门派成员',
    '重新拉取前会再确认一次，避免误触；不主动重新拉取时仍复用本次运行已读取的数据',
  ] },
  { v: '0.10.2', date: '2026-09-15', items: [
    '更新通过官方链接登录的指引，登录步骤更清晰',
    '修复个人资料说明被队徽遮挡的问题',
    '精简号码与身份的分配入口，操作更直接',
  ] },
  { v: '0.10.1', date: '2026-09-11', items: [
    '规则速查新增“赛务与申诉”分类，可查询迟到换人等局外扣分、抽局作废、同分排序和申诉时效',
    '规则速查补充本赛季新规：出局离场的附加动作、眼神暗示夜间信息、情感背书发言和第一个发言免戴麦',
    '规则速查完整收录技能发动的判罚，并更正怪盗狼王顶盾期间的结算对象和外置位投票计分',
  ] },
  { v: '0.10.0', date: '2026-09-10', items: [
    '规则速查改用《华山论剑赛事手册》2026.9.9，并补充违规扣分、版型限制和技能结算规则',
    '版型“侦探警犬”按赛事手册更名为“侦探怪盗”',
    '进入选手对比后会在后台加载深层数据并显示进度，查看身份和共同对局时等待更少',
    '凑齐 12 人后，可按版型设置号码和身份，并按名单顺序一键发号或按号码排列座次',
    '号码与身份资料卡会突出照片、号码、身份及相关表现，缺少身份数据时明确标注阵营数据来源',
    '登录页和帮助中心补充通过“四刀四神”公众号或官方选手页获取 Token 的完整步骤',
  ] },
  { v: '0.9.2', date: '2026-09-10', items: [
    ...
    'Mac 版可从电脑版微信自动检测登录信息，并跳过聊天记录等大型目录减少等待；检测失败时仍可改用手动 Token',
  ] },
...
function helpHTML() {
  const loginHelp = manualTokenOnly()
    ...
    : '<li>Windows 或 Mac 版先按“获取 Token”中的步骤，在电脑版微信进入任一选手详情页，再回到本工具点“开始实时检测”。</li>';
  const renewHelp = manualTokenOnly()
    ? '<li>Token 过期后，重新取得并粘贴一枚有效 Token。</li>'
    : '<li>登录信息过期后，回电脑版微信重新进入任一选手详情页，再回本程序重新检测。</li>';
    ...
    <div class="about-body guide-body">
        ...
        <div class="help-major-body">
          ...
          <details id="help-compare" class="help-topic">
            ...
            <div class="help-topic-body"><ol class="help-steps">
              ...
              <li>进入对比后会在后台加载所选人员的深层数据，可在赛区、赛季旁查看进度。加载期间仍可浏览概览或分配号码与身份；失败时点击“重试缺失数据”。</li>
              ...
              <li>凑齐 12 人后，点对比标题右侧的“分配号码与身份”进入独立界面并选择版型。宽屏用两排展示 12 人；资料卡以选手照片为主，旁侧醒目显示已选号码和身份，“改号码 / 改身份”固定在展示区下方同一行；点击姓名可查看个人资料。</li>
              <li>点“按顺序发号码”可按当前名单顺序一键发放 1～12 号；号码全部分配后，点“按号码排列座次”依次显示 1～12 号，再点“恢复名单顺序”即可还原。后台身份数据完成加载时不会打断正在进行的选择。</li>
              <li>每张卡可单独修改号码、身份，选择“待选”或“待选身份”可清除该项；已占用的号码和单张身份会与原持有者调整，普通狼人和平民不能超过版型名额。“清除全部号码”和“清除全部身份”互不影响；换版型只清身份，数据口径可展开“口径说明”查看。</li>
              <li>分配后及原“按身份”对比中的卡片会显示相关技能：好人看投狼、站边与警徽投票，女巫看毒狼、猎人看带狼、预言家看验狼、侦探看翻狼，狼人看悍跳次数和成功率；各项优先用身份数据，缺失时用对应阵营整体数据。狼人身份卡会使用更醒目的红色。</li>
            </ol></div>
          </details>
          ...
          <details id="help-tools" class="help-topic">
            ...
            <div class="help-topic-body"><ol class="help-steps">
              <li>“华山规则”可按分类浏览，也可输入关键词查找分数、评选、身份、版型、违规扣分和赛务申诉规则；清空关键词即可恢复当前分类的全部规则。</li>
          ...
          <details id="help-login" class="help-topic">
            ...
            <div class="help-topic-body"><ol class="help-steps">
              ...
              <li>本次运行会复用已经读取和计算过的内容，重复查看同一范围通常会更快。确认官方有更新时，个人数据、多人对比和赛事数据都可就地“重新拉取数据”获取最新结果；也可点首页“退出程序”，看到“程序已退出”后重新打开。</li>
      ...
      <details id="help-faq" class="help-major faq-section">
        ...
        <div class="faq-group"><h4>个人数据</h4><div class="faq-list">
          ...
          <details class="faq-item"><summary>为什么个人概览会提示官方数据有误？</summary><p>个人概览和逐场战绩来自两个官方接口。完整战绩读取完成后，如果官方概览总场次与逐场场数不同，页面会明确提示概览数据有误；概览仍保留官方原值，请以逐场战绩为准。</p></details>
          ...
          <details class="faq-item"><summary>身份卡为什么标注“好人整体”或“狼人整体”？</summary><p>各项指标优先用该身份的已有数据，缺失时回退到同一赛区、赛季的对应阵营数据，并在该项旁标注来源。投狼率、站对边率和悍跳数据目前主要来自阵营汇总；女巫毒狼率等技能比率直接使用已有身份指标。两者都没有时显示“—”，不会额外读取单局复盘或反推次数。进入选手对比后即后台加载身份数据，对比页和 12 人界面均显示进度。读取期间可先选择号码和身份，数据到齐后自动更新；失败时可点“重试缺失数据”。</p></details>
          <details class="faq-item"><summary>切换版型、赛区或赛季会清除分配吗？</summary><p>切换版型会清除全部身份并保留号码；版型只决定身份名额，不筛选历史战绩。切换赛区、赛季会更新资料卡的数据，保留号码和身份。返回对比或查看个人资料后再返回也会保留；移出选手会清除该人的分配，更换全部人员、清空对比或刷新页面会清除分配。</p></details>
          ...
          <details class="faq-item"><summary>为什么多人对比中的按身份和同场对比需要更久？</summary><p>进入对比后先显示每名选手的概览，同时后台读取身份表现和逐场战绩，最多同时读取 4 人；人数多或战绩多时需要等待，可在范围选择旁查看进度。新增选手只补加载新人；切换赛区或赛季后读取新范围的数据。同场对比需要等所有选手的战绩到齐后确认共同对局。</p></details>
          <details class="faq-item"><summary>切换比较视图会重新读取数据或改变比较范围吗？</summary><p>三种视图使用同一份数据，切换视图、重点选手或排序不会重复查询。后台加载完成后，身份技能比率、按身份、同场对比和号码与身份界面都会直接复用。焦点对照和隐藏只改变显示；同场对比仍按对比篮中的全部选手查找共同对局。</p></details>
        ...
        <div class="faq-group"><h4>华山工具箱</h4><div class="faq-list">
          <details class="faq-item"><summary>华山规则依据哪一版手册？</summary><p>规则速查整理自《华山论剑赛事手册》2026.9.9，每条规则末尾标注对应章节和页码；本赛季另行通知、手册尚未收录的新规也已补入，例如出局离场的附加动作和情感背书发言。手册更新后本工具会同步内容，页面标题也会显示当前依据的版本。牌局记录中的出局原因沿用工具既有名称，与手册用词是同一件事，例如“狼美人连人”对应手册的“殉情”，“石像鬼猎杀”对应手册的“带刀”。</p></details>
          <details class="faq-item"><summary>规则速查收录哪些内容，报名和奖金能查到吗？</summary><p>规则速查收录影响牌局判定、单局得分和最终名次的内容：算分与评选、牌局规则、版型规则、违规与扣分、赛务与申诉。其中“赛务与申诉”包含迟到和换人等局外扣分、季后赛与总决赛的抽局作废、同分排序，以及申诉时效和重赛错局。报名费用、赛程日历和奖金金额在手册中列为附件，请以官方公告为准。</p></details>
        ...
        <div class="faq-group"><h4>加载与缓存</h4><div class="faq-list">
          ...
          <details class="faq-item"><summary>为什么 Windows 版进入选手详情页后仍检测不到？</summary><p>程序会同时扫描旧版微信的默认 <span class="mono">LocalStorage</span> 位置，并从新版微信正在运行的 <span class="mono">WeChatAppEx.exe</span> 自动取得网页运行时目录和文件存储目录，不需要手动填写路径。微信“文件管理”中的 <span class="mono">xwechat_files</span> 主要保存聊天文件，但部分安装或迁移环境也可能在其目录树中保留网页登录数据，程序同样会自动检查。请保持选手详情页和电脑版微信打开后重新检测；仍然检测不到时，可以改用 Token 登录。</p></details>
          <details class="faq-item"><summary>为什么 Mac 版检测不到微信登录信息？</summary><p>不同微信版本保存网页登录信息的位置可能不同，macOS 也可能阻止程序读取微信数据。检测会跳过已知的聊天记录、附件和网页资源缓存目录，减少微信数据较多时的等待。请先在 Mac 微信中重新进入任一选手详情页；如果系统询问是否允许访问，请选择允许。仍然检测不到时，可以改用 Token 登录。</p></details>
          <details class="faq-item"><summary>为什么进入选手详情页后没有看到 Token？</summary><p>华山网页不会把 Token 作为文字显示出来。进入任一选手详情页后，Token 会在电脑版微信的网页登录信息中生成或更新；保持详情页打开，回到本工具点击“开始实时检测”即可。需要在另一台设备或小程序使用时，先让桌面版检测成功，再到“常用功能 → 使用说明 → 获取 Token”复制。</p></details>
          <details class="faq-item"><summary>为什么加载时要把按钮置灰？</summary><p>数据尚未齐全时继续操作，可能产生残缺结果或混入上一次的查询范围。准备完成后按钮会自动恢复，无需重复点击。</p></details>
          <details class="faq-item"><summary>为什么重新查询或再次双击程序后仍然是之前的数据？</summary><p>本次运行会固定已经读取的数据，避免同一页面前后出现不同结果。确认官方有更新时，可在个人数据、多人对比或赛事数据点“重新拉取数据”就地更新对应内容（重新拉取前会再确认一次）。也可点首页“退出程序”，看到“程序已退出”后重新打开并查询。同一版本仍在运行时，再次双击只会打开现有页面。</p></details>
        </div></div>
        </div>
      </details>

      <details id="help-token" class="help-major">
        <summary><small>03</small><span><b>获取 Token</b><em>从电脑版微信生成、检测与复制</em></span></summary>
        <div class="help-major-body help-service-body">
          <p><b>先任选一种方式，在电脑版微信中打开官方选手详情页：</b></p>
          <p><b>方式一 · 通过公众号进入</b></p>
          <ol class="help-steps">
            <li>打开并登录电脑版微信，在顶部搜索“四刀四神”，进入该公众号。</li>
            <li>在公众号中选择“门派报名”，打开华山赛事主页。</li>
            <li>主页会显示“华山城市赛区”“华山系列赛事”和“华山合作赛事”，请选择“华山城市赛区”。</li>
            <li>任意选择一个赛区，再选择“选手数据”。</li>
            <li>任意选择一名选手，点击进入选手详情页。</li>
          </ol>
          ...
          <p>${manualTokenOnly() ? '当前系统无法自动读取电脑版微信的登录信息。请先在另一台 Windows 或 Mac 设备按上述步骤操作，让桌面版检测成功，再从该设备复制有效 Token，粘贴到本机登录页完成验证。' : '保持选手详情页打开，回到本工具点击“开始实时检测”。Windows 会同时检查旧版微信的默认 LocalStorage，以及新版微信网页进程声明的运行时和文件存储位置，不需要手动填写路径。登录成功后，再打开“常用功能 → 使用说明 → 获取 Token”，即可用下方按钮复制完整 Token，供另一台设备或微信小程序临时登录。'}</p>
