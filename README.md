...
- **数据面板**：个人详情按概览 / 角色表现 / 版型表现 / 逐场战绩分区切换；个人概览可从百分比和场均分中选择 5 至 7 个维度绘制表现雷达图，角色与版型表均可按场次、场均分、胜率和评选次数排序。
...
1. Windows 或 Mac：在**电脑版微信**里打开自己的『华山战力页』并登录一次（登录状态约 1 天有效），再回到本工具点“开始实时检测”；检测时战力页可以保持打开。
2. Windows 双击 `.exe` 即可；Mac 首次运行先执行 `chmod +x <文件名>` 和 `xattr -dr com.apple.quarantine <文件名>`，再运行该文件。程序会自动打开浏览器。
3. 自动检测失败时，可粘贴由另一台已登录设备“帮助中心 → 获取 Token”中“复制当前 Token”取得的有效 Token。
4. 登录信息过期后：Windows 和 Mac 都可回微信重新登录并持续检测，也可以直接粘贴新的有效 Token。
5. 手动 Token 只保存在本次程序的内存中，关闭即清除。
6. 同一版本已经运行时，再次双击程序会直接打开现有页面；打开另一个版本时，正在运行的版本会先退出，再打开所选版本。主题和资料队徽等页面设置会在下次打开时继续使用。点首页常用功能里的「退出程序」可立即退出；只关闭浏览器页面时，程序会在约 3 分钟后自动退出。
...
桌面版使用 `v0.8.0` 形式的 Git 标签；小程序发布后使用 `miniprogram-v0.1.0` 形式的标签，避免两个产品的版本标签重名。
