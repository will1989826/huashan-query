...
- **选手搜索**：按选手名搜索，展示头像、门派与总分；英文名会补充常见大小写写法并按相关度排序，也支持直接输入**选手 ID** 直达。
- **作用域筛选**：赛区 / 赛季 / 门派三级筛选，候选项互相联动。
- **数据面板**：个人详情按概览 / 角色表现 / 版型表现 / 逐场战绩分区切换；角色与版型表均可按场次、场均分、胜率和评选次数排序。
- **赛事数据**：从首页独立进入全页面赛事查询，可按赛区 / 赛季 / 比赛类型查看门派排名、门派均分和选手排名；门派排名先显示，参赛数据计算完成后开放后两个页签，门派成员最多并发读取 8 名选手的逐场战绩；从赛事打开个人数据后可返回原内容和浏览位置。
...
- **常用功能**：首页直接提供青崖夜、朱砂笺和鱼乐会三套主题，以及使用说明、分享、更新和退出入口；复制 Token 位于使用说明的“登录与 Token”折叠项中。
