...
**前端模块**：`main → {ui, api, compare, events, draw-tool, options}`；`ui → {format, zone, api, compare, view}`；`compare → {format, zone, api, view}`；`events → {format, zone, api}`；`draw-tool → {format, zone, api}`；`api → {zone}`（只连本地）；`zone`、`format`、`view` 为无外部依赖的叶子。
