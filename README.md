...
2. 打开并登录**电脑版微信**，任选一种方式进入华山官方选手详情页：
   ...
   - **方式二 · 官方链接：**在电脑版微信的内置浏览器中打开 `https://h5.huashan.tv`（也可先将链接发送至任一微信会话再点击）；打开后同样选择“华山城市赛区 → 任一赛区 → 选手数据 → 任一选手”。
...
4. Windows 版会同时使用两套自动检测：保留旧版微信在 `%APPDATA%\Tencent` 和 `%LOCALAPPDATA%\Tencent` 下的 `LocalStorage` 检查，并从新版微信正在运行的 `WeChatAppEx.exe` 读取网页运行时目录和文件存储目录。无需手动填写路径。
5. 微信设置中“文件管理”的 `xwechat_files` 主要保存聊天文件，但部分安装或迁移环境也可能在其目录树中保留网页登录数据；程序会从微信进程自动取得并检查该位置。
6. 自动检测失败时，可粘贴由另一台已登录设备“帮助中心 → 获取 Token”中“复制当前 Token”取得的有效 Token。
7. 登录信息过期后：Windows 和 Mac 都可回电脑版微信重新进入任一选手详情页并持续检测，也可以直接粘贴新的有效 Token。
8. 手动 Token 只保存在本次程序的内存中，关闭即清除。
9. 同一版本已经运行时，再次双击程序会直接打开现有页面；打开另一个版本时，正在运行的版本会先退出，再打开所选版本。主题和资料队徽等页面设置会在下次打开时继续使用。点首页常用功能里的「退出程序」可立即退出；只关闭浏览器页面时，程序会在约 3 分钟后自动退出。

> **本次运行会复用首次读取的数据，不会自动更新。** 确认官方有更新时，个人数据、多人对比和赛事数据都可就地点“重新拉取数据”获取最新结果（重新拉取前会再确认一次）；也可点首页「退出程序」，看到「程序已退出」后重新打开。
>
...
**选手分析数据**：运行 `go run ./cmd/player-data`，默认获取 #6964、#3444、#1209、#9137、#734、#109、#73、#7781、#7598、#8528、#6078、#7668 的官方汇总和完整逐场数据，结果写入本地 `analysis-output/`。需要每日投票、技能和出局时间线时使用 `go run ./cmd/player-data -tier 2`；可用 `-replay-since 2026-01-01` 或 `-replay-max-per-player 20` 限制复盘范围，输出会记录实际覆盖率。命令优先读取本机微信登录信息，也可临时从 `HUASHAN_QUERY_TOKEN` 环境变量读取令牌；令牌只在进程内使用，不会写进结果文件。

**选手分析职责**：确定性清洗与统计由 `cmd/player-data` 完成，指标解释和打法判断维护在 `docs/knowledge/player-analysis/`，`.agents/skills/huashan-player-analysis/` 只负责任务分流与分析流程。
...
- 本次运行会复用首次读取的数据，不自动刷新；同一版本仍在运行时，再次双击只会打开现有页面。确认官方有更新时，个人数据、多人对比和赛事数据都可就地“重新拉取数据”获取最新结果；也可点首页「退出程序」，看到「程序已退出」后重新打开。
